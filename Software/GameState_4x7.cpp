...
GameState_4x7::GameState_4x7(GameDataRef data) : _data(data)
{
	eval = 0;
}

void GameState_4x7::Init()
{
	...
	this->_data->window.create(sf::VideoMode(SCREEN_WIDTH_4x7, SCREEN_HEIGHT_4x7), TITLE_4x7, sf::Style::Close | sf::Style::Titlebar);
...
void GameState_4x7::InitGridPieces()
{
...
void GameState_4x7::checkWinner()
{
	//if game is a draw
...
void GameState_4x7::HandleInput()
{
	sf::Event event;
	sf::Vector2i pos;
...
void GameState_4x7::Update(float dt)
{
		...
		else
		{
			if (this->_clock.getElapsedTime().asSeconds() > TRANSITION_TIME)
			{
				if (!reported)
				{
					...
					std::string fileName1 = "Board_4x7-"
					...
					std::string fileName2 = "Board_4x7-"
						+ this->_data->player1->playerName
						+ "vs"
						+ this->_data->player2->playerName
						+ "-"
...
void GameState_4x7::Draw(float dt)
{
