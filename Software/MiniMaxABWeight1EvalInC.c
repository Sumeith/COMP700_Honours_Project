...
void minimaxABWeight1Eval_4x4(Board_4x4* board, int depth, int alpha, int beta, Discs playerDisc, int* move, int* eval)
{
	...
	for (int col = 0; col < WIDTH_4x4; col++)
	{
		...
		if (playerDisc == PLAYER_ONE_DISC)
		{
			...
			minimaxABWeight1Eval_4x4(&tempBoard, depth - 1, alpha, beta, PLAYER_TWO_DISC, &tempMove, &tempEval);
		...
		else if (playerDisc == PLAYER_TWO_DISC)
		{
			...
			minimaxABWeight1Eval_4x4(&tempBoard, depth - 1, alpha, beta, PLAYER_ONE_DISC, &tempMove, &tempEval);
			if (tempEval < bestEval)
			{
				bestMove = col;
				bestEval = tempEval;
...
void minimaxABWeight1Eval_7x4(Board_7x4* board, int depth, int alpha, int beta, Discs playerDisc, int* move, int* eval)
{
	...
	for (int col = 0; col < WIDTH_7x4; col++)
	{
		...
		if (playerDisc == PLAYER_ONE_DISC)
		{
			...
			minimaxABWeight1Eval_7x4(&tempBoard, depth - 1, alpha, beta, PLAYER_TWO_DISC, &tempMove, &tempEval);
			if (tempEval > bestEval)
			{
				bestMove = col;
				bestEval = tempEval;
		...
		else if (playerDisc == PLAYER_TWO_DISC)
		{
			...
			minimaxABWeight1Eval_7x4(&tempBoard, depth - 1, alpha, beta, PLAYER_ONE_DISC, &tempMove, &tempEval);
			if (tempEval < bestEval)
			{
				bestMove = col;
				bestEval = tempEval;
...
void minimaxABWeight1Eval_4x7(Board_4x7* board, int depth, int alpha, int beta, Discs playerDisc, int* move, int* eval)
{
	...
	for (int col = 0; col < WIDTH_4x7; col++)
	{
		...
		if (playerDisc == PLAYER_ONE_DISC)
		{
			...
			minimaxABWeight1Eval_4x7(&tempBoard, depth - 1, alpha, beta, PLAYER_TWO_DISC, &tempMove, &tempEval);
			if (tempEval > bestEval)
			{
				bestMove = col;
				bestEval = tempEval;
		...
		else if (playerDisc == PLAYER_TWO_DISC)
		{
			...
			minimaxABWeight1Eval_4x7(&tempBoard, depth - 1, alpha, beta, PLAYER_ONE_DISC, &tempMove, &tempEval);
			if (tempEval < bestEval)
			{
				bestMove = col;
				bestEval = tempEval;
...
void minimaxABWeight1Eval_7x6(Board_7x6* board, int depth, int alpha, int beta, Discs playerDisc, int* move, int* eval)
{
	...
	for (int col = 0; col < WIDTH_7x6; col++)
	{
		...
		if (playerDisc == PLAYER_ONE_DISC)
		{
			...
			minimaxABWeight1Eval_7x6(&tempBoard, depth - 1, alpha, beta, PLAYER_TWO_DISC, &tempMove, &tempEval);
			if (tempEval > bestEval)
			{
				bestMove = col;
				bestEval = tempEval;
		...
		else if (playerDisc == PLAYER_TWO_DISC)
		{
			...
			minimaxABWeight1Eval_7x6(&tempBoard, depth - 1, alpha, beta, PLAYER_ONE_DISC, &tempMove, &tempEval);
			if (tempEval < bestEval)
			{
				bestMove = col;
				bestEval = tempEval;
...
void minimaxABWeight1Eval_6x7(Board_6x7* board, int depth, int alpha, int beta, Discs playerDisc, int* move, int* eval)
{
	...
	for (int col = 0; col < WIDTH_6x7; col++)
	{
		...
		if (playerDisc == PLAYER_ONE_DISC)
		{
			...
			minimaxABWeight1Eval_6x7(&tempBoard, depth - 1, alpha, beta, PLAYER_TWO_DISC, &tempMove, &tempEval);
			if (tempEval > bestEval)
			{
				bestMove = col;
				bestEval = tempEval;
		...
		else if (playerDisc == PLAYER_TWO_DISC)
		{
			...
			minimaxABWeight1Eval_6x7(&tempBoard, depth - 1, alpha, beta, PLAYER_ONE_DISC, &tempMove, &tempEval);
			if (tempEval < bestEval)
			{
				bestMove = col;
				bestEval = tempEval;
...
void minimaxABWeight1Eval_8x8(Board_8x8* board, int depth, int alpha, int beta, Discs playerDisc, int* move, int* eval)
{
	...
	for (int col = 0; col < WIDTH_8x8; col++)
	{
		...
		if (playerDisc == PLAYER_ONE_DISC)
		{
			...
			minimaxABWeight1Eval_8x8(&tempBoard, depth - 1, alpha, beta, PLAYER_TWO_DISC, &tempMove, &tempEval);
			if (tempEval > bestEval)
			{
				bestMove = col;
				bestEval = tempEval;
		...
		else if (playerDisc == PLAYER_TWO_DISC)
		{
			...
			minimaxABWeight1Eval_8x8(&tempBoard, depth - 1, alpha, beta, PLAYER_ONE_DISC, &tempMove, &tempEval);
			if (tempEval < bestEval)
			{
				bestMove = col;
				bestEval = tempEval;
...
void minimaxABWeight1Eval_14x12(Board_14x12* board, int depth, int alpha, int beta, Discs playerDisc, int* move, int* eval)
{
	...
	for (int col = 0; col < WIDTH_14x12; col++)
	{
		...
		if (playerDisc == PLAYER_ONE_DISC)
		{
			...
			minimaxABWeight1Eval_14x12(&tempBoard, depth - 1, alpha, beta, PLAYER_TWO_DISC, &tempMove, &tempEval);
			if (tempEval > bestEval)
			{
				bestMove = col;
				bestEval = tempEval;
		...
		else if (playerDisc == PLAYER_TWO_DISC)
		{
			...
			minimaxABWeight1Eval_14x12(&tempBoard, depth - 1, alpha, beta, PLAYER_ONE_DISC, &tempMove, &tempEval);
			if (tempEval < bestEval)
			{
				bestMove = col;
				bestEval = tempEval;
...
void minimaxABWeight1Eval_12x14(Board_12x14* board, int depth, int alpha, int beta, Discs playerDisc, int* move, int* eval)
{
	...
	for (int col = 0; col < WIDTH_12x14; col++)
	{
		...
		if (playerDisc == PLAYER_ONE_DISC)
		{
			...
			minimaxABWeight1Eval_12x14(&tempBoard, depth - 1, alpha, beta, PLAYER_TWO_DISC, &tempMove, &tempEval);
			if (tempEval > bestEval)
			{
				bestMove = col;
				bestEval = tempEval;
		...
		else if (playerDisc == PLAYER_TWO_DISC)
		{
			...
			minimaxABWeight1Eval_12x14(&tempBoard, depth - 1, alpha, beta, PLAYER_ONE_DISC, &tempMove, &tempEval);
			if (tempEval < bestEval)
			{
				bestMove = col;
				bestEval = tempEval;
